...
def solution_C5():
    ...
    # Define a callback to stop training when MAE is lower than 0.1
    class Callback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if (logs.get('mae') < 0.1):
                print("\nMAE is lower than 0.1!")
                self.model.stop_training = True

    # Instantiate the callback
    mae_callback = Callback()

    model.fit(train_set, epochs=50, validation_data=valid_set, callbacks=[mae_callback])
    return model
