...
def solution_B4():
    ...
    # Define a callback to stop training when when both accuracy and validation accuracy exceed 97%
    class Callback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if (logs.get('accuracy') > 0.91 and logs.get('val_accuracy') > 0.91):
                print("\nAccuracy and Validation Accuracy is more than 91%!")
                self.model.stop_training = True

    # Instantiate the callback
    acc_callback = Callback()

    model.fit(training_padded, training_label_seq, epochs=50,
              validation_data=(validation_padded, validation_label_seq), callbacks=[acc_callback])
